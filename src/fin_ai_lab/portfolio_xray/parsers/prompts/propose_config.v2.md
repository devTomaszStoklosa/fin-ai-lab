...
- a column mapping from the file's header names to as many of these
  ...
  asset_class, quantity, avg_cost, market_value. Give this as a list of
  entries, one per mapped column, each with the file's header name
  (file_header) and the canonical field it maps to (field) — not as a
  single object;

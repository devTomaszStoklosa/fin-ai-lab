...
- Klucz API bez podpiętego billingu — repo zostaje wyłącznie w darmowym tierze, na stałe, nie tymczasowo. `FIN_AI_LAB_MAX_RUN_COST_USD=0` jest fail-safe (patrz LLM-API.md), realną ochronę ciągłości przebiegów daje throttling dopasowany do RPM/RPD, nie budżet w dolarach.
...
- Dane wysyłane w darmowym tierze mogą być wykorzystane przez Google do trenowania modeli — przy zasadzie zero kosztów przejście na płatny tier nie jest opcją, więc do danych prywatnych (eksporty brokerów, dane osobowe) używaj wyłącznie danych syntetycznych/zanonimizowanych, patrz LLM-API.md.
